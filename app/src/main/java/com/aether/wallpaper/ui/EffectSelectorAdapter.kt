...
/**
 * RecyclerView adapter for displaying available shader effects.
 *
 * Populates effect cards dynamically from ShaderRegistry:
 * - Shader name from @shader tag
 * - Description from @description tag
 * - Tags as chips from @tags
 * - "Add Effect" button to create new layers
 *
 * Zero hardcoded shader names - all metadata-driven.
 */
class EffectSelectorAdapter(
    private val shaderRegistry: ShaderRegistry,
    private val onAddEffect: (ShaderDescriptor) -> Unit
) : RecyclerView.Adapter<EffectSelectorAdapter.EffectViewHolder>() {
